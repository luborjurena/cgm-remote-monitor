...
var palette = require('../report_plugins/palette');
...
var init = function init () {
  ...
  client.init(function loaded () {
    ...
    // ****** FOOD CODE START ******
    var food_categories = Object.create(null);
    var food_list = [];
    ...
    function fillFoodForm (event) {
      $('#rp_category').empty().append($('<option>').val('').text(translate('(none)')));
      Object.keys(food_categories).forEach(function eachCategory (s) {
        $('#rp_category').append($('<option>').val(s).text(htmlUtils.toTextContent(s)));
      });
      filter.category = '';
    ...
    function fillFoodSubcategories (event) {
      ...
      filter.subcategory = '';
      $('#rp_subcategory').empty().append($('<option>').val('').text(translate('(none)')));
      if (filter.category !== '') {
        Object.keys(food_categories[filter.category] || {}).forEach(function eachSubCategory (s) {
          $('#rp_subcategory').append($('<option>').val(s).text(htmlUtils.toTextContent(s)));
        });
      }
      doFoodFilter();
      return maybePrevent(event);
    ...
    function doFoodFilter (event) {
      ...
      for (var i = 0; i < food_list.length; i++) {
        ...
        $('#rp_food').append($('<option>').val(food_list[i]._id).text(htmlUtils.toTextContent(o)));
      }
    ...
    $.ajax('/api/v1/food/regular.json', {
      ...
      , success: function foodLoadSuccess (records) {
        records.forEach(function(r) {
          food_list.push(r);
          if (r.category && !Object.prototype.hasOwnProperty.call(food_categories, r.category)) {
            food_categories[r.category] = Object.create(null);
          }
    ...
    function sgvToColor (sgv, options) {
      if (sgv > options.targetHigh) {
        return palette.tiers.high;
      } else if (sgv < options.targetLow) {
        return palette.tiers.low;
      }
      return palette.tiers.inRange;
    }

    // Export the currently shown report to PDF. The @media print stylesheet
    // (report.css) hides the page chrome and lays the active report out for A4;
    ...
    function processData (data, day, options, callback) {
      ...
      data.sgv = data.sgv.concat(data.mbg.map(function(obj) { return { date: new Date(obj.mills), y: obj.y, sgv: client.utils.scaleMgdl(obj.y), color: palette.mbg, type: 'mbg', device: obj.device } }));
