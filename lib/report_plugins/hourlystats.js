...
hourlystats.html = function html (client) {
  ...
  var ret =
    '<h2 class="rp-h2">' + translate('Hourly stats') + '</h2>' +
    '<div class="rp-card rp-scroll"><div id="hourlystats-overviewchart"></div></div>' +
    '<div id="hourlystats-report"></div>';
  return ret;
};

hourlystats.css =
  '#hourlystats-overviewchart{width:100%;min-width:6.5in;height:380px;}' +
  '#hourlystats-placeholder td{text-align:center;}';
...
hourlystats.report = function report_hourlystats (datastorage, sorteddaystoshow, options) {
  ...
  data.forEach(function(record) {
    ...
    var d = profile.applyTimezone(moment(record.displayTime));
    record.sgv = Number(record.sgv);
    pivotedByHour[d.hour()].push(record);
  });

  var table = $('<table class="rp-table">');
  var thead = $('<tr/>');
  ...
  report.empty();
  report.append($('<div class="rp-card rp-scroll">').append(table));
  ...
  $.plot(
    ...
    , [{
      data: stats
      , candle: {
        risingColor: palette.bandInner
        , fallingColor: palette.bandInner
        , strokeColor: palette.median
        , lineWidth: 1
        , maxWidth: 24
      }
    }], {
  ...
  sorteddaystoshow.forEach(function(day) {
    if (datastorage[day].netBasalPositive) {
      ...
      for (var h = 0; h < 24; h++) {
        ...
        totalNet[h] += net;
        var klass = Math.abs(net) < 0.019 ? 'rp-zero' : (net < 0 ? 'rp-neg' : 'rp-pos');
        $('<td>' +
          '<span class="rp-muted">' + negative.toFixed(2) + '</span>' + '<br>' +
          '<span class="rp-muted">' + positive.toFixed(2) + '</span>' + '<br>' +
          '<span class="' + klass + '">' + net.toFixed(2) + '</span>' +
          '</td>').appendTo(tr);
      }
      table.append(tr);
    }
  ...
  if (days > 0) {
    var tr = $('<tr class="rp-table-total">');
    $('<td>' + translate('Average') + " " + days + " " + translate('days') + '</td>').appendTo(tr);
    for (var h = 0; h < 24; h++) {
      var klass = Math.abs(totalNet[h]) < 0.01 ? '' : (totalNet[h] < 0 ? 'rp-bad' : 'rp-good');
      $('<td class="' + klass + '">' +
        '<span class="rp-muted">' + (totalNegative[h] / days).toFixed(2) + ' (' + negativesCount[h] + ')' + '</span>' + '<br>' +
        '<span class="rp-muted">' + (totalPositive[h] / days).toFixed(2) + ' (' + positivesCount[h] + ')' + '</span>' + '<br>' +
        '<span>' + (totalNet[h] / days).toFixed(2) + '</span>' +
        '</td>').appendTo(tr);
    }
    table.append(tr);
  }

  report.append($('<div class="rp-card">')
    .append('<div class="rp-card-title">' + translate('netIOB stats') + '</div>')
    .append('<div class="rp-hint">' + translate('(temp basals must be rendered to display this report)') + '</div>')
    .append($('<div class="rp-scroll">').append(table)));
};
