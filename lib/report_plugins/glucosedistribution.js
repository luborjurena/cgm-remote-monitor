...
glucosedistribution.html = function html (client) {
  ...
  var ret =
    ...
    '<div class="agp-legend-item"><span class="agp-legend-swatch" style="background:var(--rp-band-inner)"></span>25–75%</div>' +
    '<div class="agp-legend-item"><span class="agp-legend-swatch" style="background:var(--rp-band-outer)"></span>5–95%</div>' +
    '<div class="agp-legend-item"><span class="agp-legend-swatch" style="background:rgba(0,131,0,.25)"></span>' + translate('Target Range') + '</div>' +
    '</div>' +
    '</div>' +
...
glucosedistribution.css =
  '.gd-report{color:var(--rp-ink);padding:4px 2px 24px;}' +
  '.gd-report *{box-sizing:border-box;}' +
  '.gd-h2{font-size:20px;font-weight:700;letter-spacing:-.01em;margin:6px 2px 16px;}' +
  '.gd-h2-sub{font-size:14px;font-weight:400;color:var(--rp-ink-3);}' +
  '.gd-card{background:var(--rp-surface);border:1px solid var(--rp-border);border-radius:var(--rp-radius);' +
  '  box-shadow:var(--rp-shadow);padding:16px 18px;margin:0 0 16px 0;}' +
  '.gd-card-title{font-size:12px;font-weight:600;letter-spacing:.05em;text-transform:uppercase;' +
  '  color:var(--rp-ink-3);margin:0 0 12px;}' +
  ...
  '  margin:0 0 14px;font-size:13px;color:var(--rp-ink-2);}' +
  '.gd-rangemetrics-title{font-size:12px;font-weight:600;letter-spacing:.05em;' +
  '  text-transform:uppercase;color:var(--rp-ink-3);margin-right:4px;}' +
  '.gd-rangemetrics label{cursor:pointer;display:inline-flex;align-items:center;gap:5px;margin:0;}' +
  // TIR bar: 2px gaps in the surface colour separate the segments
  '#glucosedistribution-tirbar{display:flex;gap:2px;height:40px;width:100%;' +
  '  border-radius:10px;overflow:hidden;margin:4px 0 14px;background:var(--rp-surface);}' +
  ...
  '#glucosedistribution-tirlegend{display:flex;flex-wrap:wrap;gap:8px 24px;font-size:13px;color:var(--rp-ink-2);}' +
  '.gd-legend-item{display:flex;align-items:center;gap:6px;}' +
  '.gd-legend-item b{color:var(--rp-ink);}' +
  '.gd-legend-swatch{width:14px;height:14px;border-radius:4px;display:inline-block;}' +
  // metric tiles: a coloured top rule carries the status, the value stays ink
  '.gd-tiles{display:grid;grid-template-columns:repeat(auto-fit,minmax(118px,1fr));gap:12px;}' +
  '.gd-tile{position:relative;overflow:hidden;padding:14px 14px 12px;border-radius:12px;' +
  '  background:var(--rp-tint);border:1px solid var(--rp-hairline);}' +
  '.gd-tile-mark{position:absolute;left:0;top:0;right:0;height:3px;}' +
  '.gd-tile-value{font-size:23px;font-weight:700;line-height:1.05;color:var(--rp-ink);}' +
  '.gd-tile-label{font-size:11px;color:var(--rp-ink-3);margin-top:5px;line-height:1.3;}' +
  // AGP curve
  '#glucosedistribution-agp{width:100%;}' +
  '.agp-svg{width:100%;height:auto;aspect-ratio:1100/320;display:block;}' +
  '.agp-target-band{fill:var(--rp-target-band);}' +
  '.agp-band-outer{fill:var(--rp-band-outer);opacity:.85;}' +
  '.agp-band-inner{fill:var(--rp-band-inner);opacity:.8;}' +
  '.agp-median{fill:none;stroke:var(--rp-median);stroke-width:2;stroke-linejoin:round;stroke-linecap:round;}' +
  '.agp-target-line{stroke:var(--rp-in-range);stroke-width:1;stroke-dasharray:5 4;opacity:.7;}' +
  '.agp-axis path,.agp-axis line{stroke:var(--rp-axis);}' +
  '.agp-axis text{fill:var(--rp-ink-3);font-size:11px;}' +
  '.agp-legend{display:flex;flex-wrap:wrap;gap:14px 24px;margin:10px 0 0;font-size:12px;color:var(--rp-ink-2);}' +
  ...
  '.agp-legend-line{width:16px;height:0;border-top:2px solid var(--rp-median);}' +
  // tables
  '.gd-report table{border-collapse:collapse;width:100%;font-size:13px;font-variant-numeric:tabular-nums;}' +
  '.gd-report th{font-size:11px;font-weight:600;letter-spacing:.02em;color:var(--rp-ink-3);' +
  '  padding:6px 8px;border-bottom:2px solid var(--rp-hairline);text-align:center;vertical-align:bottom;}' +
  '#glucosedistribution-placeholder .tdborder{border:none;border-bottom:1px solid var(--rp-hairline);' +
  ...
  '.gd-tier-range{color:var(--rp-ink-3);font-weight:400;}' +
  '.gd-overall .tdborder{border-top:2px solid var(--rp-hairline);font-weight:600;}' +
  // notes + hours
  '.gd-notes{font-size:12px;line-height:1.6;color:var(--rp-ink-3);}' +
  '.gd-notes a{color:var(--rp-accent);}' +
  ...
  '  text-transform:uppercase;color:var(--rp-ink-3);}' +
  '.gd-hours-boxes{font-size:12px;color:var(--rp-ink-2);}' +
...
glucosedistribution.report = function report_glucosedistribution (datastorage, sorteddaystoshow, options) {
  ...
  var tierColors = {
    veryLow: palette.tiers.veryLow
    , low: palette.tiers.low
    , inRange: palette.tiers.inRange
    , high: palette.tiers.high
    , veryHigh: palette.tiers.veryHigh
  };
  // tiers with a light fill need dark text for readable contrast
  var darkTextTiers = { high: true, veryHigh: true };
  function tierTextColor (key) { return darkTextTiers[key] ? palette.ink : '#fff'; }
  ...
  glucose_data = data = glucose_data2.filter(function(r) {
    return enabledHours[profile.applyTimezone(moment(r.displayTime)).hour()]
  });
  ...
  function tile (value, name, sub, valueColor) {
    ...
    if (valueColor) t.append($('<span class="gd-tile-mark">').css('background', valueColor));
    t.append($('<div class="gd-tile-value">').text(value));
  ...
  var gmiMeanGlucose = meanMgdl;
  var GMI = (Math.round(10 * (3.31 + 0.02392 * gmiMeanGlucose)) / 10).toFixed(1);
  var revisedGMI = (Math.round(10 * (2.879 + 0.023 * gmiMeanGlucose)) / 10).toFixed(1);
