...
success.html = function html (client) {
  ...
  var ret =
    '<h2 class="rp-h2">' + translate('Weekly Distribution') + '</h2>' +
    '<div class="rp-card rp-scroll" id="success-grid"></div>';
  return ret;
};

success.css =
  '#success-placeholder th:first-child{width:30%;text-align:left;}' +
  '#success-placeholder td:first-child{text-align:left;color:var(--rp-ink-2);white-space:nowrap;}';
...
success.report = function report_success (datastorage, sorteddaystoshow, options) {
  ...
  if (quarters === 0) {
    // insufficent data
    grid.append('<p class="rp-hint">' + translate('There is not sufficient data to run this report. Select more days.') + '</p>');
    return;
  }
  ...
  table.append('<tbody>' + quarters.filter(function(quarter) {
    ...
    var INVERT = true;
    return '<tr>' + [
      profile.applyTimezone(moment(quarter.starting)).format('L') + ' - ' + profile.applyTimezone(moment(quarter.ending)).format('L')
      , {
      ...
      if (typeof v === 'object') {
        return '<td class="' + (v.klass ? 'rp-' + v.klass : '') + '">' + v.text + '</td>';
      } else {
        return '<td>' + v + '</td>';
      }
    }).join('') + '</tr>';
