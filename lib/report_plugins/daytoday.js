...
daytoday.css =
  '#daytodaycharts{overflow-x:auto;}' +
  '.rp-summary{display:flex;flex-wrap:wrap;gap:6px 18px;font-size:13px;color:var(--rp-ink-2);}' +
  '.rp-summary b{color:var(--rp-ink);}';

daytoday.html = function html (client) {
  const reportStorage = require('../report/reportstorage');
  var translate = client.translate;
  function opt (id, key, label, color) {
    return '<label><input type="checkbox" id="' + id + '"' + (reportStorage.getValue(key) ? ' checked' : '') + '>' +
      (color ? '<span class="rp-swatch" style="background:' + color + '"></span>' : '') +
      '<span>' + label + '</span></label>';
  }
  var ret =
    '<h2 class="rp-h2">' + translate('Day to day') + '</h2>' +
    '<div class="rp-card">' +
    '<div class="rp-hint">' + translate('To see this report, press SHOW while in this view') + '</div>' +
    '<div class="rp-options">' +
    '<span class="rp-options-title">' + translate('Display') + '</span>' +
    opt('rp_optionsinsulin', 'insulin', translate('Insulin'), palette.insulin) +
    opt('rp_optionscarbs', 'carbs', translate('Carbs'), palette.carbs) +
    opt('rp_optionsbasal', 'basal', translate('Basal rate'), palette.basal) +
    opt('rp_optionsnotes', 'notes', translate('Notes')) +
    opt('rp_optionsfood', 'food', translate('Food')) +
    opt('rp_optionsraw', 'raw', translate('Raw'), palette.raw) +
    opt('rp_optionsiob', 'iob', translate('IOB'), palette.insulin) +
    opt('rp_optionscob', 'cob', translate('COB'), palette.carbs) +
    opt('rp_optionspredicted', 'predicted', translate('Predictions'), palette.event) +
    opt('rp_optionsopenaps', 'openAps', translate('OpenAPS'), palette.event) +
    opt('rp_optionsdistribution', 'insulindistribution', translate('Insulin distribution'), palette.insulin) +
    opt('rp_optionsbgcheck', 'bgcheck', translate('BG Check'), palette.mbg) +
    opt('rp_optionsothertreatments', 'othertreatments', translate('View all treatments')) +
    '</div>' +
    '<div class="rp-options">' +
    ...
    '</div>' +
    `<div id="rp_predictedSettings" class="rp-options" ${reportStorage.getValue('predicted') ? '' : 'style="display:none"'}>` +
    '<label>' + translate('Truncate predictions: ') +
    `<input type="checkbox" id="rp_optionsPredictedTruncate" ${reportStorage.getValue('predictedTruncate') ? "checked" : ""}></label>` +
    '<span>' + translate('Predictions offset') + ': <b><label id="rp_predictedOffset"></label> minutes</b></span>' +
...
daytoday.prepareHtml = function daytodayPrepareHtml (sorteddaystoshow) {
  ...
  sorteddaystoshow.forEach(function eachDay (d) {
    $('#daytodaycharts').append($('<table class="rp-chart-card"><tr><td><div id="daytodaychart-' + d + '"></div></td><td><div id="daytodaystatchart-' + d + '"></td></tr></table>'));
  });
};
...
daytoday.report = function report_daytoday (datastorage, sorteddaystoshow, options) {
  ...
  if (options.insulindistribution) {
    var html = '<div class="rp-card rp-summary"><b>' + translate('TDD average') + ':</b> ' + tddAverage.toFixed(1) + 'U&nbsp; ';
    ...
    html += '<b>' + translate('Fat average') + ':</b> ' + fatAverage.toFixed(0) + 'g</div>';
  ...
  function drawChart (day, data, options) {
    ...
    function prepareContextCircles (sel) {
      ...
      sel.attr('cx', function(d) {
        ...
        .attr('fill', function(d) {
          if (d.color === 'gray') {
            return options.raw ? palette.raw : 'transparent';
          }
          return d.color;
        })
        ...
        .attr('stroke', function(d) { return (options.openAps && d.openaps && d.type !== 'mbg') ? palette.ink : '#ffffff'; })
        .attr('r', function(d) {
          if (d.type === 'mbg') {
            return 4;
          } else {
        ...
        .on('mouseover', function(d) {
          if (options.openAps && d.openaps) {
            ...
            var text = '<b>BG:</b> ' + bg +
              ', ' + htmlUtils.textAsHtml(d.openaps.suggested.reason) +
              (d.openaps.suggested.mealAssist ? ' <b>Meal Assist:</b> ' + htmlUtils.textAsHtml(d.openaps.suggested.mealAssist) : '');
      ...
      if (predictions.length > 0 && treatmentsTimestamps.length > 0) {
        ...
        for (var treatmentsIndex = 0; treatmentsIndex < treatmentsTimestamps.length; treatmentsIndex++) {
          ...
          if (predictedIndex != null) {
            ...
            var end = profile.parseInTimezone(day).endOf('day');
    ...
    var from = profile.parseInTimezone(day);
    ...
    if (options.basal) {
      var basalTimes = profile.getBasalRenderTimes(from.valueOf(), to.valueOf(), times.mins(5).msecs);

      basalTimes.forEach(function renderBasal (date, index) {
        var nextDate = index + 1 < basalTimes.length ? basalTimes[index + 1] : to.valueOf();
        var durationHours = Math.max(0, Math.min(nextDate, to.valueOf()) - date) / times.hour().msecs;
        var hournow = profile.applyTimezone(moment(date)).hour();
        var basalvalue = profile.getTempBasal(date);

        // Calculate basal stats over the interval until the next sample/boundary.
        baseBasalInsulin += basalvalue.basal * durationHours;
        var tempPart = (basalvalue.tempbasal - basalvalue.basal) * durationHours;
        if (tempPart > 0) {
        ...
        // Compare basal values using JSON.stringify for deep object comparison
        if (JSON.stringify(lastbasal) !== JSON.stringify(basalvalue)) {
    ...
    if (iobStatusAvailable) {
      ...
      iobArray?.forEach(function drawCob (point) {
        if (previousdate !== 0 && point.mills - previousdate > times.mins(15).msecs) {
          iobpolyline += ', ';
    ...
    if (options.iob) {
      context.append('polyline')
        .attr('stroke', palette.insulin)
        .attr('stroke-width', 1.5)
        .attr('fill', palette.insulin)
        .attr('opacity', '0.8')
    ...
    if (options.cob) {
      context.append('polyline')
        .attr('stroke', palette.carbs)
        .attr('stroke-width', 1.5)
        .attr('fill', palette.carbs)
        .attr('opacity', '0.8')
        .attr('fill-opacity', '0.08')
    ...
    data.treatments.forEach(function(treatment) {
      ...
      if (treatment.boluscalc && treatment.boluscalc.foods && treatment.boluscalc.foods.length > 0 || renderNotes) {
        ...
        if (treatment.boluscalc && treatment.boluscalc.foods && treatment.boluscalc.foods.length > 0 && options.food) {
          ...
          for (var fi = 0; fi < foods.length; fi++) {
            ...
            context.append('text')
              ...
              .text(htmlUtils.toTextContent(text));
        ...
        if (renderNotes && !treatment.duration && treatment.eventType !== 'Temp Basal') {
          context.append('text')
            ...
            .attr('y', foodtexts * 15 + padding.top)
            .attr('transform', 'translate(' + (xScale2(treatment.mills) + padding.left) + ',' + padding.top + ')')
            .text(htmlUtils.toTextContent(treatment.notes));
          foodtexts = (foodtexts + 1) % 6;
          drawpointer = true;
        }
        if (drawpointer) {
      ...
      if (treatment.carbs && options.carbs) {
        ...
        context.append('text')
          ...
          .text(htmlUtils.toTextContent(label));
      }
      ...
      if (treatment.insulin || treatment.carbs || treatment.eventType == 'Temp Basal' || treatment.eventType == 'Combo Bolus') {
        ...
        context.append('text')
         ...
         context.append('text')
           ...
           .attr('x', xScale2(treatment.mills + times.mins(treatment.duration).msecs / 2) + padding.left)
           .text(htmlUtils.toTextContent(treatment.reason));
        ...
        context.append('text')
          ...
          .text(htmlUtils.toTextContent(translate(client.careportal.resolveEventName(treatment.eventType))));
      } else if (treatment.duration && options.othertreatments) {
    ...
    if (options.insulindistribution) {
      var boluscolor = palette.insulin;
      ...
      var table = $('<table class="rp-table rp-kv">');
      ...
      var carbscolor = d3.scaleOrdinal().range([palette.carbs]);
