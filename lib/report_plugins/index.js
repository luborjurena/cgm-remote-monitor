...
function init(ctx) {
  ...
  function plugins(name) {
    if (name) {
      return allPlugins && Array.isArray(allPlugins) ?
        allPlugins.find(plugin => plugin && plugin.name === name) : null;
    } else {
      return plugins;
    }
  }
  ...
  plugins.addHtmlFromPlugins = function addHtmlFromPlugins(client) {
    // shared data colours as --rp-* variables for every plugin stylesheet
    $('<style>')
      .prop('type', 'text/css')
      .html(require('./palette').css)
      .appendTo('head');
