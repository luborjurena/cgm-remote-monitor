...
dailystats.html = function html (client) {
  ...
  var ret =
    '<h2 class="rp-h2">' + translate('Daily stats report') + '</h2>' +
    '<div class="rp-card rp-scroll"><div id="dailystats-report"></div></div>';
  return ret;
};

dailystats.css =
  '#dailystats-placeholder .tdborder{min-width:60px;}' +
  '#dailystats-placeholder .inlinepiechart{width:2.2in;height:0.9in;}';
...
dailystats.report = function report_dailystats (datastorage, sorteddaystoshow, options) {
  ...
  report.empty();
  var table = $('<table class="rp-table centeraligned">');
  report.append(table);
  var thead = $('<tr/>');
  ...
  sorteddaystoshow.forEach(function(day) {
    ...
    $.plot(
      ...
      , inrange, {
        ...
        , colors: [palette.tiers.low, palette.tiers.inRange, palette.tiers.high]
      }
    );
  });
